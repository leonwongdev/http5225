...
<body>
  ...
  for ($i = 0; $i < count($links); $i++) {
    $linName = ($links[$i]['name'] === '' ? "Default Name" : $links[$i]['name']);
    $linkURL = ($links[$i]['url'] === '' ? "https://www.google.com/" : $links[$i]['url']);
    $imageSrc = ($links[$i]['image'] === '' ? "mozilla.png" : $links[$i]['image']);
    $linkDescription = ($links[$i]['description'] === '' ? "Default Description" : $links[$i]['description']);
    echo '<h1>' . $linName . '</h1>';
    echo '<p><a href="' . $linkURL . '" target="_blank">' . 'Visit the website' . '</a></p>';
